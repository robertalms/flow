...
def get_options():
    ...
    optParser.add_option("-v", "--verbose", action="store_true", dest="verbose",
                         default=True, help="tell me what you are doing")
    optParser.add_option("-d", "--debug", action="store_true", dest="debug",
                         default=True, help="debug messages")
...
#    "--queue-output", "outputQueue%s"%options.suffix])
    print(sys.path)
    sumo_args = ["-c", "uc5.sumo.cfg", "-r", "routes_trafficMix_0_trafficDemand_1_driverBehaviour_OS_seed_0.xml", "-a", "additionalsOutput_trafficMix_0_trafficDemand_1_driverBehaviour_OS_seed_0.xml, vTypesCAVToC_OS.add.xml, vTypesCVToC_OS.add.xml, vTypesLV_OS.add.xml","-v","True"]
...
#     
#     exp= BaselineRunnerScenario(
#     name="template",
#     net_params=net_params,
#     vehicles=vehicles
#     )
#     _ = exp.run(downwardEdgeID, distance, tocInfos) #, upwardEdgeID, upwardDist)
    run(downwardEdgeID, distance, tocInfos)
